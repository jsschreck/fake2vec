...
def fig_window(_id=0, scale_x=1, scale_y=1):
    ...
    fig = plt.figure(
        _id,
        figsize=(scale_x * fig_width, scale_y * fig_height),
        facecolor='w',
        edgecolor='k')
...
def plot_confusion_matrix(model,
    ...
    for k, (_y_true, _y_pred, classes) in enumerate(
            zip(y_true, y_pred, class_list)):
        ...
        cm = confusion_matrix(
            _y_true.argmax(axis=1),
            _y_pred.round().argmax(axis=1),
            labels=labels)
        print("... finished building confusion matrix for class {}".format(
            output_labels[k]))
        ...
        plt.tight_layout()
        PDF_SAVECHOP("results/confusion_matrix_{}_{}".format(
...
if __name__ == '__main__':
    ...
    with open("data/plot_data.pkl", "rb") as fid:
        history, X_train, y_input, X_test, y_input_test = pickle.load(fid)
