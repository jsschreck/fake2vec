...
def class_imbalance(df, label_index):
    ...
    if label_index == 0:
        ax = sns.countplot(
            data=df, x=label, order=df[label].value_counts().index, log=True)
    else:
        ax = sns.countplot(
    ...
    if label_index == 1:
        ax.set_xticklabels(
            le.inverse_transform(range(3)), rotation=15, ha="right")
    if label_index == 2:
        ax.set_xticklabels(
            le.inverse_transform(range(7)), rotation=15, ha="right")
...
if __name__ == '__main__':
    ...
    with open("data/plot_data.pkl", "rb") as fid:
        history, N_classes, X_train, y_input, X_test, y_input_test = pickle.load(
            fid)
